...
Baseline utilizes protocol-owned liquidity—neutral liquidity owned and managed by smart contracts—to ensure a 
...
external balance sheet somewhere inaccessible to the average holder.

A bToken's BLV is calculated based on the highest price at which the socialized liquidity could buy back 
the entire token supply. Put in a different context, if every holder decided to sell their bTokens, 
the BLV is the price that the last token would be sold at.
