...
Baseline's market making strategy naturally grow the value of its underlying token's BLV over time through a series of liquidity operations. Whenever the token's price moves beyond a certain threshold, anyone can permissionlessly rebalance protocol-owned liquidity across three Uniswap V3 positions.
...
Since tokens deposited in the credit facility cannot be sold into the pool, lending the token's BLV cannot cause the protocol to accrue bad debt or impact its ability to fulfill liquidity obligations. Additionally, the system doesn't require any market-driven liquidation for loans as the price can never go below the borrowed value. Loans in Baseline's credit facility can only default if borrowers let them expire, by neglecting to repay the borrowed amount within the duration of the loan.

When a borrower fails to repay their loan, Baseline repossesses their deposited tokens, treating the situation the same as if the borrower had sold their tokens at the BLV price. This means that when a loan expires, the borrower forgoes any market premium in the token's BLV, redistributing the value back to remaining holders.
