...
Unlike standard loans, where users have to pay interest based on the amount borrowed, Baseline's 
looping facility uses a "rebasing" mechanism to charge fees on looped positions. In this system, 
the protocol automatically adjusts both debt and bToken balance over time, gradually "shrinking" 
both collateral and debt, allowing users to maintain non-liquidatable leveraged exposure without 
any active management of the positino.
...
In this fee structure, users pay based on the net premium value of the position, not the borrowed 
amount. As a result, the final fees accrued by the user is directly proportional to the premium of 
the bToken when the position is closed. Users that exit a looped position at higher premiums will 
pay more fees for their positio; at lower premiums, users will pay less. 

### BLV Value Accrual

Looped positions don't generate direct cash flows to the afterburner for buybacks, but looping still 
positively impacts a bToken's BLV due to the supply reduction from "shrinking" user's collateral. 
